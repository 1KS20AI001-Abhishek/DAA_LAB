 ...
 */
    ...
    // Find the vertex with minimum value from the set of vertices not yet included in MST\
    ...
    static int vertex_mincost_edge(int weight[], Boolean VT[], int V) {
        ...
        for (int v = 0; v < V; v++)
            if (VT[v] == false && weight[v] < minwt) {
                minwt = weight[v];// insert code here
                min_index = v; // insert code here
            }
        return min_index;// insert code here
    }
...
// insert code here);
    ...
    static Boolean findPrimsMST(int graph[][], int V, int start, int parent[]) {
        ...
        for (int v = 0; v < V; v++) {
            ...
            parent[v]= start;// insert code here
        }

        // in each iteration, pick one vertex corresponding to min cost edge
        for (int count = 0; count < V - 1; count++) {
            // Select the minimum value vertex from the set of vertices not yet included in MST
            ...
            if (u == -1) { // graph is disconnected
                ...
                for (int v = 0; v < V; v++) {
                    if (graph[u][v] != 0 && !VT[v] && graph[u][v] < weight[v]) {
                        parent[v] = u;
                        weight[v] = graph[u][v];// insert code here
                    }
                }
            } // if else
        }// end for count
...
// insert code here
    ...
    public static void main (String[] args) throws FileNotFoundException {
        ...
        if ( findPrimsMST(graph, V, 0, parent) ) {
            endtime = System.nanoTime();
            MST_print(parent, graph, V, 0 );
            //insert code here to call findPrimsMST) {
            // Call MST_print
            // insert code here
            System.out.println("Time taken: " + (endtime - start_time) + " nsecs");
