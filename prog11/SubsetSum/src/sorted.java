...
public class sorted {
     ...
     */
    public static void findElements(int size, int[] elem, int[] x, int index, int currsum, int tgtsum) {
        if (tgtsum == currsum) {// solution found with current index
            display(size, elem, x); // display the current subset.
            x[index] = 0;// no need to explore further with this element
            return;
        }
        // if this is the last element, no more exploration
        if (index == size-1 && tgtsum != currsum+elem[index]) {
            return;
        }
        ...
        if(x[index] == 1) {
            ...
            if (currsum <= currsum + elem[index + 1]) {
                x[index + 1] = 1;
